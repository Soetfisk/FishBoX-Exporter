...
void FishBoX::printShit()
{
	...
	for (int i = 0; i < HEADER.meshCount; i++)
	{
		...
		printf("\nBlendShapes: %d", meArray->blendshapesCount);
		...
		for (int j = 0; j < FBX.GetMeshVec()[i].vertexData.size(); j++) //vertexData
		{
			...
			printf("Position\n X: %f, Y: %f, Z: %f\n", FBX.GetMeshVec()[i].vertexData[i].x,
				FBX.GetMeshVec()[i].vertexData[i].y,
				FBX.GetMeshVec()[i].vertexData[i].z);
...
void FishBoX::writeShit()
{
	...
	{
		outfile.write((const char*)meArray, sizeof(mesh));
